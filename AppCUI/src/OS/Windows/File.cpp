...
#define VALIDATE_FILE_HANLDE(returnValue) CHECK(this->FileID.Handle!=INVALID_HANDLE_VALUE,returnValue,"File has not been opened !");
...
{
    CHECK(filePath, false, "Expecting a valid (non-null) file path !");
    Close();
    HANDLE hFile = CreateFileA(filePath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    CHECK(hFile != INVALID_HANDLE_VALUE, false, "Fail to create: %s ==> Error code: %d", filePath, GetLastError());
    CHECK(SetFilePointer(hFile, 0, NULL, FILE_END)!= INVALID_SET_FILE_POINTER, false, "Fail to set file pointer at the end of the file !");
...
bool                File::OpenRead(const char * filePath)
{
    CHECK(filePath, false, "Expecting a valid (non-null) file path !");
    Close();
    HANDLE hFile = CreateFileA(filePath, GENERIC_READ , FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    CHECK(hFile != INVALID_HANDLE_VALUE, false, "Fail to create: %s ==> Error code: %d", filePath, GetLastError());
    this->FileID.Handle = hFile;
    return true;
}
bool                File::Create(const char * filePath, bool overwriteExisting)
{
    CHECK(filePath, false, "Expecting a valid (non-null) file path !");
    Close();
    HANDLE hFile = CreateFileA(filePath, GENERIC_READ|GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, overwriteExisting? CREATE_ALWAYS: CREATE_NEW, 0, NULL);
    CHECK(hFile != INVALID_HANDLE_VALUE, false, "Fail to create: %s ==> Error code: %d", filePath, GetLastError());
    this->FileID.Handle = hFile;
    return true;
}
...
bool                File::ReadBuffer(void* buffer, unsigned int bufferSize, unsigned int & bytesRead)
{
    ...
    DWORD nrBytesRead = 0;
    CHECK(ReadFile(F_HNDL, buffer, bufferSize, &nrBytesRead, NULL), false, "Reading from file failed with code: %d", GetLastError());
