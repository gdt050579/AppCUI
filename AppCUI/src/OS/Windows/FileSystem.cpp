...
bool FileSystem::EnumerateFiles(
      AppCUI::Utils::String& path,
      bool recursive,
      EnumerateFilesCallback callback,
      void* Context,
      bool callFolderCallbackAfterProcessingAllOfItsFiles)
{
    ...
    CHECK(hFindFile != INVALID_HANDLE_VALUE,
          false,
          "Invalid path (%s) - ErrorCode: %08X",
          path.GetText(),
          GetLastError());
    do
    {
        ...
        {
            CHECK(Path::Join(path, dt.cFileName),
                  false,
                  "Unable to add file name (%s) to path variable !",
                  dt.cFileName);
            CHECK(callback(
                        path.GetText(),
                        dt.cFileName,
                        (((unsigned long long) dt.nFileSizeHigh) << 32) | ((unsigned long long) dt.nFileSizeLow),
                        false,
                        Context),
                  false,
                  "Callback stop by user !");
            CHECK(path.Truncate(len), false, "Unable to truncate path variable !");
        }
        else
        {
            if (((Utils::String::Equals(dt.cFileName, ".") == false) &&
                 (Utils::String::Equals(dt.cFileName, "..") == false)))
            {
                CHECK(Path::Join(path, dt.cFileName),
                      false,
                      "Unable to add directory name (%s) to path variable !",
                      dt.cFileName);
                ...
                if (recursive)
                {
                    CHECK(EnumerateFiles(
                                path, recursive, callback, Context, callFolderCallbackAfterProcessingAllOfItsFiles),
                          false,
                          "Error on reading files from folder: %s",
                          path.GetText());
                }
                CHECK(path.Truncate(len), false, "Unable to truncate path variable !");
            }
...
{
    CHECK(::SetFileAttributesA(fullPath, FILE_ATTRIBUTE_NORMAL),
          false,
          "Failed to clear attributes for '%s' -> with error: %08X",
          fullPath,
          GetLastError());
    CHECK(::RemoveDirectoryA(fullPath),
          false,
          "Failed to delete folder for '%s' -> with error: %08X",
          fullPath,
          GetLastError());
    return true;
}
bool FileSystem::DeleteFile(const char* name, bool failIfFileIsMissing)
{
    ...
    CHECK(::SetFileAttributesA(name, FILE_ATTRIBUTE_NORMAL),
          false,
          "Failed to clear attributes for '%s' -> with error: %08X",
          name,
          GetLastError());
...
bool FileSystem::GetSpecialFolderName(SpecialFolder id, AppCUI::Utils::String& name)
{
    ...
    case SpecialFolder::DriveZ:
        CHECK(name.SetFormat("%c:\\", (((char) id) - (char) SpecialFolder::DriveA) + 'A'),
              false,
              "Fail to create special folder name !");
