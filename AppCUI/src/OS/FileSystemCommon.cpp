...
bool FileSystem::DeleteFolder(
      AppCUI::Utils::String& name, bool failIfFileIsMissing, DeleteFileCallback callback, void* Context)
{
...
bool FileSystem::CopyFile(const char* source, const char* destination, CopyFileCallback callback, void* Context)
{
    ...
    while (true)
    {
        ...
        while ((readSoFar < sourceSize) && (continue_copy))
        {
            ...
            CHECKBK(
                  fs.Read(tempBuffer, (unsigned int) toRead),
                  "Fail to read %d bytes from %s",
                  (unsigned int) toRead,
                  source);
            CHECKBK(
                  fd.Write(tempBuffer, (unsigned int) toRead),
                  "Fail to write %d bytes to %s",
                  (unsigned int) toRead,
                  destination);
            readSoFar += toRead;
