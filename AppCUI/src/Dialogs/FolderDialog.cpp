...
constexpr unsigned int ALL_FILES_INDEX = 0xFFFFFFFF;
...
int FolderDialog_ListViewItemComparer(
...
bool FolderDialogClass::ProcessExtensionFilter(const char* start, const char* end)
{
    ...
    while (p < end)
    {
        p = FolderDialogUtils::SkipSpaces(p, end);
        n = FolderDialogUtils::SkipUntilChar(p, end, ':');
        ...
        p = FolderDialogUtils::SkipSpaces(n + 1, end);
        ...
        if ((*p) == '[')
        {
            ...
            tempSet.clear();
            p++;
            end_array = FolderDialogUtils::SkipUntilChar(p, end, ']');
            CHECK(end_array < end, false, "Premature end of an array list (missing ']' delimiter)");

            // parse the array
            while (p < end_array)
            {
                n = FolderDialogUtils::SkipUntilChar(p, end_array, ',');
                tempSet.insert(FolderDialogUtils::__compute_hash__(p, n));
                p = n + 1;
            }
            p = end_array + 1;
            p = FolderDialogUtils::SkipSpaces(p, end); // skip any possible spaces
            if ((p < end) && ((*p) == ','))
...
void FolderDialogClass::UpdateCurrentExtensionFilter()
{
...
void FolderDialogClass::UpdateFileList()
{
            ...
            for (const auto& fileEntry : std::filesystem::directory_iterator(p))
            {
                ...
                else
                {
                    ...
                    if (extFilter)
                    {
                        ...
                        if ((ext != nullptr) && ((*ext) == '.'))
                            ext++;
                        unsigned int hash = FolderDialogUtils::__compute_hash__(ext, ext_end);
                        if (!extFilter->contains(hash))
                            continue; // extension is filtered
                    }
                    FolderDialogUtils::ConvertSizeToString((unsigned long long) fileEntry.file_size(), size);
                }

                auto lastModifiedTime = FolderDialogUtils::getLastModifiedTime(fileEntry);
...
bool FolderDialogClass::OnEventHandler(const void* sender, AppCUI::Controls::Event eventType, int controlID)
{
    switch (eventType)
    {
      ...
      const std::filesystem::path& _path)
{
    ...
    wnd.SetEventHandler(FolderDialog_EventHandler, this);
    ...
    files.SetItemCompareFunction(FolderDialog_ListViewItemComparer, this);
...
std::optional<std::filesystem::path> FolderDialog::ShowSaveFileWindow(
