...
#define COMPUTE_RGB(r,g,b) ( (((unsigned int)((r) & 0xFF))<<16) | (((unsigned int)((g) & 0xFF))<<8) | ((unsigned int)((b) & 0xFF)) )
#define COMPUTE_ARGB(a,r,g,b) ( (((unsigned int)((a) & 0xFF))<<24) | (((unsigned int)((r) & 0xFF))<<16) | (((unsigned int)((g) & 0xFF))<<8) | ((unsigned int)((b) & 0xFF)) )
...
#define CHECK_INDEX(errorValue) \
	...
	CHECK(x < Width, errorValue, "Invalid X (%d) value. It should be less than %d (the width of the image)",x,Width); \
	CHECK(y < Height, errorValue, "Invalid Y (%d) value. It should be less than %d (the height of the image)",y,Height); \
	pixel = &Pixels[y*Width + x]; 

#define VALIDATE_CONSOLE_INDEX CHECK(((unsigned int)color) < 16, false, "Invalid console color index (should be between 0 and 15)");
...
bool Image::SetPixel(unsigned int x, unsigned int y, unsigned char Red, unsigned char Green, unsigned char Blue, unsigned char Alpha)
{
	CHECK_INDEX(false);
