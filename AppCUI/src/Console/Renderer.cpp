...
bool Renderer::WriteSingleLineTextWithHotKey(int x, int y, const char * text, unsigned int width, const ColorPair color, const ColorPair hotKeyColor, unsigned int hotKeyOffset, TextAlignament align, int textSize)
{
    if (width == 0)
        return false; // nothing to print
    CHECK(text, false, "Expecting a valid (non-null) text ");
